...
def preprocess_gk(series, colname):
    split = series.fillna('').str.split(',', expand=True).iloc[:, :3]
    ...
    try:
        ...
        for _, row in ratings_df.iterrows():
            try:
                ...
                prod_company = tmdb_row['production_companies'].iloc[0].split(",")[0] if tmdb_row['production_companies'].notnull().any() else ''
                prod_country = tmdb_row['production_countries'].iloc[0].split(",")[0] if tmdb_row['production_countries'].notnull().any() else ''
